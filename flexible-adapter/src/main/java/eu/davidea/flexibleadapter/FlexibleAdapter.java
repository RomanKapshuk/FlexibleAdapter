...
public class FlexibleAdapter<T extends IFlexible>
	...
	private List<T> mItems, mTempItems, mOriginalItems;
	 ...
	 * Default value is {@code false}, always {@code true} when filter is active.
	 *
	...
	/**
	 * <p>This method filters the internal list with the searchText previously set with
	 * {@link #setSearchText(String)}.</p>
	 * Use when all items are loaded with No endless scrolling.
	 *
	 ...
	 * {@link #setSearchText(String)}.</p>
	 * Use when all items are loaded with No endless scrolling.
	 * <p>Same as {@link #filterItems()}, but with a delay in the execution, useful to grab
	...
	public void filterItems(@IntRange(from = 0) long delay) {
		filterItems(new ArrayList<T>(), delay);
	}
	 ...
	 * more characters from user before starting the search.</p>
	 *
	 * @param unfilteredItems a custom list to filter
	 * @param delay           any non-negative delay
	 * @see #filterItems()
	 * @see #filterItems(long)
	 * @see #filterItems(List)
	...
	private synchronized void filterItemsAsync(@NonNull List<T> unfilteredItems) {
		...
		// Use current list as original list if unfilteredItems is not provided
		if (hasSearchText() && mOriginalItems == null && unfilteredItems.isEmpty()) {
			mOriginalItems = mItems;
			unfilteredItems = mOriginalItems;
		} else if (mOriginalItems != null) {
			unfilteredItems = mOriginalItems;
		}
